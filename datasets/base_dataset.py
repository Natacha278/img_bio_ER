...
from datasets.dataset import PainDatasets, PainImgBioDatasets
...
####################################################################################################
####################################################################################################
    def load_bio_img_dataset(dataset_img_path,label_path_train, label_path_val, batch_size, phase):
        try: 
            val_idx = []
            train_idx = []
            shuffle = False
            ...
            dataset_train = PainImgBioDatasets(dataset_img_path, label_path_train, transform=transform_dict[phase])
            
            """
                condition to split train and val if no validation is defined
            """
            if label_path_val is not None:
                dataset_val = PainImgBioDatasets(dataset_img_path, label_path_val)
                shuffle = True
            else:
                np.random.seed(SEED)
                shuffled_indices = np.random.permutation(len(dataset_train))
