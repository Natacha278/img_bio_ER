...
def main(args, target_subject):
    ...
    #Testing before adaptation
    source_trained_model = torch.load(config.CURRENT_DIR + '/' + source_model_name  + '_load.pt')
    transfer_model.load_state_dict(source_trained_model['model_state_dict'])

    acc_noDA = test(transfer_model, dataloaders['tar_test'], args.batch_size, args.n_class, args.is_pain_dataset)
    print(f'Target Test Accuracy before DA: {acc_noDA}')
    results_file.write(f'Target Test Accuracy before DA: {acc_noDA}\n')
    ...
    # acc = test(transfer_model, dataloaders['tar'], args.batch_size, args.n_class, args.is_pain_dataset)
    # acc_val = test(transfer_model, dataloaders['tar_val'], args.batch_size, args.n_class, args.is_pain_dataset)
    acc_test = test(transfer_model, dataloaders['tar_test'], args.batch_size, args.n_class, args.is_pain_dataset)

    # print(f'Target Accuracy: {acc}')
    # print(f'Target Val Accuracy: {acc_val}')
    print(f'Target Test Accuracy: {acc_test}')
    results_file.write(f'Target Test Accuracy: {acc_test}\n')
...
def train_multi_model(n_epoch, model, data_loader1, data_loader2, optimizer, criterion, n_batch, trained_model_name, val_loader, train_source):
    ...
    for e in range(n_epoch):
        ...
        if train_source is False:
            if e % 20 == 0:
                output_file.write(f"epoch: {e}\n")
                output_file.write(f"nb_tar conf -1: {len(tar_loader_for_PL) *  args.batch_size}\n" ) ###
                _data_arr, _prob_arr, _label_arr, _gt_arr = generate_tar_aug_conf_pl(model, tar_loader_for_PL, threshold)
                output_file.write(f"nb_tar conf 0: {_data_arr.shape}\n")###
                ...
                for i in range(0,1):
                    ...
                    output_file.write(f"nb_tar conf 1: {_data_arr.shape}\n")###
                ...
                output_file.write(f"nb_tar for next PL:{len(tar_loader_for_PL) *  args.batch_size} \n")###
        ...
        acc = test(model, val_loader, args.batch_size, args.n_class, args.is_pain_dataset)
...
def test(model, target_test_loader, batch_size, n_class, is_pain_dataset=False):
    model.eval()
    correct = 0
    corr_acc_top1 = 0
...
if __name__ == '__main__':
    ...
    output_file = open("methods/output_tests.txt", "a")
    results_file = open("methods/results.txt", "a")
    results_file.write(" ---- Orgin msbda ----\n")
    results_file.write(f" targets: {target_list_name}\n")
    for tar_sub in target_list_name:
        results_file.write(f" --target: {tar_sub}--\n")
        main(args, tar_sub)
    results_file.write("\n")
    output_file.close()
    results_file.close()
